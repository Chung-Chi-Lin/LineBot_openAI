...
const COMMANDS_MAP = {
    ...
    司機: {
        ...
        乘客資訊: {
            ...
            remark: '取得"目前乘客名稱與更改資訊ID，ID為需要修改資訊時才會使用到" (輸入範例> 乘客資訊)',
        },
        乘客車資計算: {
...
async function passengerFareCount(profile, event) {
    const inputMatch = event.message.text.match(/^(\w+)\s([\+\-]\d+)\s*備註\s?(.*)$/);
    ...
    const driverData = await executeSQL('SELECT * FROM users WHERE line_user_driver = ?', [profile.userId]);
...
async function handleEvent(event) {
    ...
    if (validationResult.type === 'existing_user') {
        ...
        const command =
            ...
            COMMANDS_MAP[userLineType][inputText];
